...
gridsample_grad2 = load(
    ...
    sources=[os.path.join(curr_path, "gridsample_cuda.cpp"), os.path.join(curr_path, "gridsample_cuda.cu")],
    verbose=True,
)
...
class _GridSample2dForward(torch.autograd.Function):
    ...
    def forward(ctx, input, grid, padding_mode=0, align_corners=True):
        ...
        output = torch.nn.functional.grid_sample(
            input=input, grid=grid, mode="bilinear", padding_mode=padding_mode, align_corners=align_corners
        )
...
class _GridSample2dBackward(torch.autograd.Function):
    ...
    def backward(ctx, grad2_grad_input, grad2_grad_grid):
        ...
        out = gridsample_grad2.grad2_2d(
            grad2_grad_input, grad2_grad_grid, grad_output, input, grid, ctx.padding_mode, ctx.align_corners
        )
